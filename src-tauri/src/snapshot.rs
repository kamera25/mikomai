...
impl SnapshotManager {
    pub fn new(app: &tauri::AppHandle) -> anyhow::Result<Self> {
        ...
        let mut base_dir = app
            .path()
            .app_data_dir()
            .map_err(|e| anyhow::anyhow!("Failed to retrieve app data directory: {}", e))?;
...
mod tests {
    ...
    fn test_snapshot_creation_and_merge() {
        ...
        let snap_dir1 = manager
            .create_snapshot_dir()
            .expect("Failed to create snap dir 1");
        assert!(snap_dir1.exists());

        let file_a_1 = manager
            .save_artifact("Router-A", "config", "Router-A config version 1")
            .expect("Failed to save Router-A config");
        let file_b_1 = manager
            .save_artifact("Router-B", "config", "Router-B config version 1")
        ...
        manager
            .update_current_link(&snap_dir1)
            .expect("Failed to update current for snap 1");
        ...
        let snap_dir2 = manager
            .create_snapshot_dir()
            .expect("Failed to create snap dir 2");
        // Ensure they are unique directories
        assert_ne!(snap_dir1, snap_dir2);

        let file_a_2 = manager
            .save_artifact("Router-A", "config", "Router-A config version 2")
        ...
        manager
            .update_current_link(&snap_dir2)
            .expect("Failed to update current for snap 2");
    ...
    fn test_data_type_extension() {
        ...
        let file_txt = manager
            .save_artifact("device1", "config", "content")
            .unwrap();
        assert_eq!(file_txt.file_name().unwrap(), "device1_config.txt");

        // Test keeping custom extension
        let file_json = manager
            .save_artifact("device1", "arp.json", "content")
            .unwrap();
