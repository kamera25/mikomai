...
impl LlmWorker for BuilderWorker {
    ...
    fn ask(
        ...
        log::info!(
            "BuilderWorker: subsequent_task to be sent: {:?}",
            subsequent_task_owned
        );

        let (prompt, modified_user_message, output, history_block) =
            ...
            if let Some(w) = window {
                    ...
                    if let Ok(connections) = crate::connections::load_connections_raw(app_handle) {
                        if let Some(conn) = connections.iter().find(|c| c.matches_host_or_ip(&host))
                        {
                            let device_type_str = conn
                                .device_type
                                .as_ref()
                                .map(|d| d.as_str().to_lowercase())
                                .unwrap_or_default();
                            let vendor_type_str = conn
                                .vendor_type
                                .as_ref()
                ...
                if !is_cisco_ios {
                    let user_msg_lower = user_message
                        ...
                        .unwrap_or_default();
                    let prompt_lower = prompt
                        .as_ref()
                        .map(|s| s.to_lowercase())
                        .unwrap_or_default();

                    let target_vendors = if user_msg_lower.contains("arista")
                    ...
                    for vendor in target_vendors {
                        ...
                        let conv_res = rt.block_on(async {
                            crate::mcp::config_helper::convert_cisco_config(
                                config.clone(),
                                vendor.clone(),
                            )
                            .await
...
pub(crate) fn extract_config_block(text: &str) -> Option<String> {
    ...
    if target_text.contains("interface")
        || target_text.contains("hostname")
        || target_text.contains("vlan ")
    {
