...
impl AstValidator {
    ...
    fn validate_node(&self, path: &str, value: &Value, schema_node: &SchemaNode, errors: &mut Vec<ValidationError>) {
        match schema_node.node_type.as_str() {
            "container" => {
                if let Value::Mapping(map) = value {
                    ...
                    errors.push(ValidationError {
                        path: path.to_string(),
                        message: format!("Type mismatch: expected container, got {}", Self::get_type_name(value)),
                    });
                }
            }
            "integer" => {
                if let Value::Number(num) = value {
                        ...
                        if let Some(range_str) = &schema_node.range {
                            if let Some((min, max)) = Self::parse_range(range_str) {
                                if i < min || i > max {
                                    errors.push(ValidationError {
                                        path: path.to_string(),
                                        message: format!("Value {} is out of range {}", i, range_str),
                                    });
                                }
                            } else {
                                ...
                                errors.push(ValidationError {
                                    path: path.to_string(),
                                    message: format!("Invalid range definition in schema: '{}'", range_str),
                                });
                            }
                        }
                    } else {
                    ...
                    errors.push(ValidationError {
                        path: path.to_string(),
                        message: format!("Type mismatch: expected integer, got {}", Self::get_type_name(value)),
                    });
                }
            }
            "string" => {
                if let Value::String(s) = value {
                    if let Some(enum_vals) = &schema_node.enum_values {
                        if !enum_vals.contains(s) {
                            errors.push(ValidationError {
                                path: path.to_string(),
                                message: format!("Value '{}' is not in allowed enum {:?}", s, enum_vals),
                            });
                        }
                    }
                } else {
                    errors.push(ValidationError {
                        path: path.to_string(),
                        message: format!("Type mismatch: expected string, got {}", Self::get_type_name(value)),
                    });
                }
            }
            unknown_type => {
...
"#;
        ...
        assert!(errors.is_empty(), "Expected no errors for valid YAML, got {:?}", errors);
    }
...
"#;
        ...
        assert!(errors.iter().any(|e| e.path == "vlan_config.vlan-id" && e.message.contains("out of range")));

        // Verify enum error
        assert!(errors.iter().any(|e| e.path == "vlan_config.interface-mode" && e.message.contains("not in allowed enum")));

        // Verify unknown property error
        assert!(errors.iter().any(|e| e.path == "vlan_config.status" && e.message.contains("Unknown property")));
    }
...
"#;
        ...
        assert_eq!(errors.len(), 2, "Expected 2 validation errors (vlan-id and interface-mode missing)");

        assert!(errors.iter().any(|e| e.path == "vlan_config.vlan-id" && e.message.contains("Missing required property")));
        assert!(errors.iter().any(|e| e.path == "vlan_config.interface-mode" && e.message.contains("Missing required property")));
    }
}
