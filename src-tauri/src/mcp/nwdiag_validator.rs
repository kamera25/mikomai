...
impl NwdiagValidationError {
    pub fn new(line: Option<usize>, message: impl Into<String>, suggestion: Option<impl Into<String>>) -> Self {
        Self {
            line,
...
fn validate_statement(
    ...
    let s = stmt.trim();
    if s.is_empty() || s == "nwdiag" || s == "network" || s.starts_with("network ") || s == "group" || s.starts_with("group ") {
        return Ok(());
    }
    ...
    if content.contains('=') && !content.contains('[') {
        ...
        let allowed_keys = [
            "address", "color", "textcolor", "shape", "label", "description",
            "fontsize", "width", "height", "stacked", "class"
        ];
        if !allowed_keys.contains(&key) {
            return Err(NwdiagValidationError::new(
                Some(line_num),
                format!("Unknown property '{}' in statement", key),
                Some(format!("Supported properties include: {}", allowed_keys.join(", "))),
            ));
        }
    ...
    if let Some(bracket_start) = content.find('[') {
        ...
        if node_name.is_empty() {
            return Err(NwdiagValidationError::new(
                ...
                Some("Specify a node identifier before attributes, e.g. router01 [shape = router];"),
            ));
        }
...
fn validate_attributes(attrs: &str, line_num: usize) -> Result<(), NwdiagValidationError> {
    ...
    let allowed_shapes = [
        "box", "square", "roundedbox", "dots", "circle", "ellipse",
        "diamond", "minidiamond", "note", "mail", "cloud", "actor",
        "beginpoint", "endpoint", "condition", "database"
    ];
    ...
    for item in items {
        ...
        if !allowed_attr_keys.contains(&k) {
            return Err(NwdiagValidationError::new(
                ...
                Some(format!("Supported attributes: {}", allowed_attr_keys.join(", "))),
            ));
        }
...
fn is_valid_identifier(ident: &str) -> bool {
    ...
    ident.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-' || c == '.')
}
...
mod tests {
    ...
    fn test_missing_nwdiag_wrapper() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().message.contains("Schema must start with 'nwdiag {'"));
    }
    ...
    fn test_empty_network_block() {
        ...
        let res = validate_nwdiag_schema(schema);
        assert!(res.is_err());
        assert!(res.unwrap_err().message.contains("Network block cannot be empty"));
    }
