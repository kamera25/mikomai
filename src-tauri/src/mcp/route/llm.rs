        ...
        match serde_yaml::from_str::<crate::schema::route::UniversalRouteTable>(&clean_yaml) {
                ...
                match parsed.validate() {
                    ...
                    Err(validation_errors) => {
                        ...
                        if retry_count < max_retries {
                            ...
                            return Err(format!("Route table validation failed after {} retries. Errors: {:?}", max_retries, validation_errors));
                        }
                    }
                }
            }
            Err(parse_err) => {
                ...
                if retry_count < max_retries {
                    ...
                    return Err(format!("Route table parsing failed after {} retries. Error: {}", max_retries, parse_err));
                }
            }
        }
    }
