...
fn clean_yaml_output(output: &str) -> String {
    ...
    if cleaned.ends_with("```") {
        cleaned = cleaned[..cleaned.len() - "```".len()].trim_end().to_string();
    }
...
5. Do NOT include any conversational text or explanations.",
    ...
    while retry_count <= max_retries {
        println!("Prompting LLM to convert ARP table (Attempt {})...", retry_count + 1);
        ...
        match serde_yaml::from_str::<crate::schema::arp::UniversalArpTable>(&clean_yaml) {
            Ok(parsed) => {
                match parsed.validate() {
                    Ok(_) => {
                        println!("ARP table successfully parsed and validated!");
                    ...
                    Err(validation_errors) => {
                        ...
                        if retry_count < max_retries {
                            ...
                            return Err(format!("ARP table validation failed after {} retries. Errors: {:?}", max_retries, validation_errors));
                        }
                    }
                }
            }
            Err(parse_err) => {
                ...
                if retry_count < max_retries {
                    ...
                    return Err(format!("ARP table parsing failed after {} retries. Error: {}", max_retries, parse_err));
                }
            }
        }
    }
