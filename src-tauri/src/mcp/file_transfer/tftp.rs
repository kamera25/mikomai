...
pub async fn tftp_upload_core(
    ...
    while chunk_idx < chunks.len() || (chunk_idx == chunks.len() && data.len() % TFTP_BLOCK_SIZE == 0) {
...
pub async fn network_tftp_download_with_params(
    ...
    let remote_file = params
        .remote_file
        .or(params.filename)
        .ok_or_else(|| "Error: remote_file or filename is required for TFTP download".to_string())?;
    ...
    let (file_data, duration_ms) = tftp_download_core(server_addr, &remote_file, mode, timeout_duration).await?;
    ...
    let saved_path_str = if let Some(local_path) = params.local_path {
        ...
        fs::write(&path, &file_data)
            .map_err(|e| format!("Failed to write downloaded TFTP file to {}: {}", local_path, e))?;
        local_path
    } else {
...
pub async fn network_tftp_upload_with_params(
    ...
    let bytes_len = file_data.len();
    let duration_ms = tftp_upload_core(server_addr, &remote_file, &file_data, mode, timeout_duration).await?;
