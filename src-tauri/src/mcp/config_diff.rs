...
pub fn normalize_config_for_diff(config: &str) -> String {
    ...
    let lines: Vec<&str> = config
        ...
        .filter(|line| {
            ...
            !IGNORABLE_KEYWORDS
                .iter()
                .any(|keyword| lower.starts_with(keyword) || lower.contains(keyword))
...
mod tests {
    ...
    fn normalization_removes_cli_noise_and_outer_blank_lines() {
        let input = "\nInfo: generated\nshow run\ninterface Gi0/1  \n description uplink\n\n";
        assert_eq!(
            normalize_config_for_diff(input),
            "interface Gi0/1\n description uplink"
        );
    }
    ...
    fn diff_tracks_insertions_and_deletions() {
        ...
        assert_eq!(
            diff.iter().filter(|line| line.r#type == "insert").count(),
            1
        );
        assert_eq!(
            diff.iter().filter(|line| line.r#type == "delete").count(),
            1
        );
    }
}
