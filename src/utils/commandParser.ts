export function parsePingCommand(input: string) {
  ...
  const pingBaseMatch =
    lowerInput.match(/(?:ping|ピン|ピング)\s+([a-zA-Z0-9.:-]+)/) ||
  ...
  const countMatch =
    lowerInput.match(/(?:count|回数|回)\s*(\d+)/) || lowerInput.match(/(\d+)\s*回(?:実行)?/);
  if (countMatch) args.count = parseInt(countMatch[1]);

  if (
    lowerInput.includes("df") ||
    lowerInput.includes("フラグメント禁止") ||
    lowerInput.includes("断片化禁止")
  ) {
    args.df = true;
  }
