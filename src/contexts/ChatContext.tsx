...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const commitHistoryMutation = (mutation: HistoryMutation) => {
    ...
    const mutationPromise = historyMutationQueue.current.then(async () => {
      const snapshot = await ipc.command<HistorySnapshot>(COMMANDS.mutateHistory, { mutation });
  ...
  useEffect(() => {
    const initHistory = async () => {
      try {
      const snapshot = await ipc.command<HistorySnapshot>(COMMANDS.initializeHistory);
  ...
  useEffect(() => {
    const initSummaries = async () => {
      try {
      const savedSummaries = await ipc.command<SummaryItem[]>(COMMANDS.loadSummaries);
