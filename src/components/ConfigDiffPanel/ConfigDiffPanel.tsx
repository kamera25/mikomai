...
function useConfigDiffPresenter({ id, isOpen, onClose, style, isResizing }: ConfigDiffPanelProps) {
  ...
  const {
    ...
    toggleStepCollapse,
    handleCommit,
    handleForceCommitChoice,
  } = useConfigDiffExecution({ id, isOpen, proposedDiffData });
  const diffData = verifiedDiffData || proposedDiffData;
  const selectTab = (tab: "diff" | "logs") => setActiveTab(tab);
  const toggleStep = (key: string) => toggleStepCollapse(key);
  ...
  const renderStatusBadge = () => {
    switch (phase) {
      case "fetching_before":
        return (
          <span className="diff-phase-badge info">
            <RefreshIcon size={14} className="spinning" /> 現状Config取得中...
          </span>
        );
      case "dry_running":
        return (
          <span className="diff-phase-badge warning">
            <FlaskIcon size={14} /> 自動Dry-run検証中...
          </span>
        );
      case "deploying":
        return (
          <span className="diff-phase-badge warning">
            <RocketIcon size={14} /> Netmiko投入中...
          </span>
        );
      case "verifying":
        return (
          <span className="diff-phase-badge info">
            <SearchIcon size={14} /> Diff検証中...
          </span>
        );
      case "success":
        return (
          <span className="diff-phase-badge success">
            <CheckIcon size={14} /> 投入&Diff検証完了
          </span>
        );
      case "failed":
        return (
          <span className="diff-phase-badge error">
            <AlertCircleIcon size={14} /> エラー発生
          </span>
        );
      default:
        return null;
      ...
      <div className="diff-content">
        ...
        {forceCommitReq && (
          ...
          >
            ...
            <div style={{ fontSize: "0.85rem", marginBottom: "8px" }}>{forceCommitReq.message}</div>
                ...
                steps.map((step) => {
                  ...
                  return (
                    <div key={step.key} className={`log-step-group ${step.status}`}>
                      <div className="log-step-header" onClick={() => toggleStep(step.key)}>
                        <ChevronIcon
                          size={14}
                        ...
                        <div className="log-step-icon">
                          {step.status === "running" && <div className="log-step-spinner" />}
                          {step.status === "success" && (
                          ...
                          {step.status === "pending" && <div className="log-step-pending-dot" />}
                        </div>
                        ...
                        <div className="log-step-body">
                          {step.logs.map((log, index) => {
                            ...
                            if (log.startsWith("[STATUS]"))
                              className = "diff-log-line diff-log-status";
                            else if (log.startsWith("[SYSTEM]"))
                              className = "diff-log-line diff-log-system";
                            else if (
                              log.includes("Error") ||
                              log.includes("FAILED") ||
                              log.startsWith("[DRY-RUN ERROR]")
                            )
                              className = "diff-log-line diff-log-error";
                            else if (log.startsWith("[DRY-RUN OK]"))
                              className = "diff-log-line diff-log-success";
        ...
        {phase === "idle" ? (
          ...
          >
            <span
              style={{
                fontSize: "0.85rem",
                color: phase === "success" ? "var(--success)" : "var(--danger)",
              }}
            >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", width: "100%" }}>
            ...
            />
            <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)", flexGrow: 1 }}>
              {statusMessage}
            </span>
