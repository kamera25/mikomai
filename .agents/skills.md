...
1. **正常終了の確認**:
   - ログに `[AgentLoop] Step X: Goal reached / Completed:` または最終回答のテキストが出力されていること。
   - 回答内容が質問（プロンプト）に対して妥当かつ完結していること。

2. **終了コードに関する注意（macOS / Metal 環境）**:
   - macOS 上でローカル LLM（llama.cpp）を使用している場合、プロセス終了処理時（Metal デバイス解放時）に `GGML_ASSERT` が発生し終了コード 134 (abort) となる場合があります。
   - この場合でも、直前にエージェントの推論結果・回答テキストが正常に出力されていれば、バックエンドおよびエージェントロジック自体の動作検証としては成功とみなして構いません。
